...
begin
  ...
  if selected_org is not null and not exists (
    ...
    where user_id=auth.uid() and organization_id=selected_org and status='active'
  ) then
    raise exception 'You do not have active access to that organization' using errcode='42501';
  end if;

  if selected_org is null then
    select count(*), min(organization_id)
      into active_org_count, selected_org
    from public.organization_members
    where user_id=auth.uid() and status='active';
    if active_org_count <> 1 then selected_org := null; end if;
  end if;
...
set search_path = ''
as $$
  select public.create_support_case(case_category,case_description,null::uuid)
$$;
