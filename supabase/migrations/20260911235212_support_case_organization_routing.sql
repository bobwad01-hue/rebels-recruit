create or replace function public.support_case_organization_options()
returns table(organization_id uuid, organization_name text, relationship text)
language sql
security definer
set search_path = ''
as $$
  with allowed as (
    select om.organization_id, 'member'::text as relationship
    from public.organization_members om
    where om.user_id = auth.uid() and om.status = 'active'
    union
    select athlete_org.organization_id, 'parent'::text as relationship
    from public.parent_guardian_access pga
    join public.organization_members athlete_org
      on athlete_org.user_id = pga.athlete_user_id
     and athlete_org.status = 'active'
    where pga.parent_user_id = auth.uid()
      and pga.status = 'active'
  )
  select a.organization_id,o.name,
         case when bool_or(a.relationship='member') then 'member' else 'parent' end
  from allowed a
...
revoke all on function public.support_case_organization_options() from public;
grant execute on function public.support_case_organization_options() to authenticated;
...
begin
  ...
  if selected_org is null then
    select count(*), min(x.organization_id)
      into allowed_org_count, selected_org
    from public.support_case_organization_options() x;
    if allowed_org_count <> 1 then selected_org := null; end if;
  end if;
