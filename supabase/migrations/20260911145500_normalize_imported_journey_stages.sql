create or replace function public.import_pipeline_stage(raw text)
returns public.pipeline_stage
language plpgsql immutable set search_path='public' as $$
declare s text:=lower(btrim(coalesce(raw,'')));
begin
  return case
    when s in ('target','target school') then 'Target'::public.pipeline_stage
    when s='contacted' then 'Contacted'::public.pipeline_stage
    when s='engaged' then 'Engaged'::public.pipeline_stage
    when s in ('interested','recruiting interest','interest') then 'Recruiting Interest'::public.pipeline_stage
    when s in ('high interest','highly interested') then 'High Interest'::public.pipeline_stage
    when s in ('visit/camp','visit','camp') then 'Visit/Camp'::public.pipeline_stage
    when s in ('offer','decision') then 'Decision'::public.pipeline_stage
    when s='committed' then 'Committed'::public.pipeline_stage
    else 'Researching'::public.pipeline_stage
  end;
end $$;
