...
export function buildRecruitingIntelligence(input:IntelligenceInput){
 ...
 openMoves.forEach((x:any)=>{
  ...
  if(overdue){signals.push({id:`overdue-${x.id}`,kind:'urgent',title:x.title||'Overdue Next Move',detail:'This Next Move is late. Finish it or choose a new due date so it does not get lost.',priority:115,href:'/game-plan#next-moves'});return}
  const timing=daysUntil===0?'due today':daysUntil===1?'due tomorrow':daysUntil!==null?`due in ${daysUntil} days`:'ready when you are';
  signals.push({id:`next-${x.id}`,kind:'opportunity',title:x.title||'Complete your next recruiting step',detail:`This is one of your current Next Moves and is ${timing}. Open Game Plan to see the context and take action.`,priority:daysUntil===0?112:daysUntil===1?110:daysUntil!==null&&daysUntil<=3?108:daysUntil!==null&&daysUntil<=7?104:96,href:'/game-plan#next-moves'});
 });
