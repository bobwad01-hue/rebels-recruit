...
export function buildRecruitingIntelligence(input:IntelligenceInput){
 ...
 const upcoming=events.filter(x=>(!x.rsvp||String(x.rsvp).toLowerCase()==='going')&&x.date&&day(x.date)!==null&&day(x.date)!>=todayValue).sort((a,b)=>String(a.date).localeCompare(String(b.date)));
 ...
 coaches.forEach((r:any)=>{const c=one(r.college_coaches),school=one(r.colleges),coachRows=interactions.filter((x:any)=>x.coach_id&&x.coach_id===r.coach_id).sort((a:any,b:any)=>String(b.date||b.created_at||'').localeCompare(String(a.date||a.created_at||''))),latest=coachRows[0],days=age(r.last_contact_date||latest?.date||latest?.created_at),coachName=[c?.first_name,c?.last_name].filter(Boolean).join(' ')||'this coach';if(latest&&String(latest.initiated_by).toLowerCase()==='coach'&&days!==null&&days<=14)signals.push({id:`reply-${r.id}`,kind:'urgent',title:`Respond to ${coachName}`,detail:`${school?.name||'School'} · The latest logged interaction came from the coach.`,priority:109,href:c?.id?`/coaches/${c.id}`:'/connections'});if(days!==null&&days>=30)signals.push({id:`cool-${r.id}`,kind:'urgent',title:`Follow up with ${coachName}`,detail:`${school?.name||'School'} · ${days} days since your last recorded contact.`,priority:95,href:c?.id?`/coaches/${c.id}`:'/connections'});else if(days!==null&&days<=14)signals.push({id:`warm-${r.id}`,kind:'momentum',title:`Keep building with ${coachName}`,detail:`${school?.name||'School'} · You have had recent contact.`,priority:72,href:c?.id?`/coaches/${c.id}`:'/connections'})});
 upcoming.slice(0,3).forEach((e:any)=>{const days=Math.ceil((day(e.date)!-Date.now())/86400000);if(days<=14)signals.push({id:`event-${e.id}`,kind:'opportunity',title:`Get ready for ${e.name}`,detail:days<=0?'This event is today.':`This event is in ${days} day${days===1?'':'s'}.`,priority:days<=3?100:80,href:`/events/${e.id}/prep`})});
