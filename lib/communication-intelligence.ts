import {exactInteractionDate,isExactDatedInteraction} from '@/lib/interaction-dates';

export type InteractionLike={id?:string;athlete_user_id?:string;coach_id?:string|null;college_id?:string|null;type?:string|null;date?:string|null;date_precision?:string|null;date_year?:number|null;date_month?:number|null;created_at?:string|null;note?:string|null;initiated_by?:string|null;colleges?:any;college_coaches?:any};
...
export function interactionMeaningScore(i:InteractionLike){const t=lower(i?.type);const n=lower(i?.note);let score=4;
 ...
 const exactDate=exactInteractionDate(i);if(exactDate){const age=daysSince(exactDate);if(age<=7)score+=4;else if(age<=30)score+=2}
 return Math.min(20,score);
}

export function isMeaningfulContact(i:InteractionLike){return interactionMeaningScore(i)>=9}
export function isCoachResponse(i?:InteractionLike|null){if(!i)return false;const text=`${lower(i.type)} ${lower(i.note)} ${lower(i.initiated_by)}`;return /coach/.test(lower(i.initiated_by))||/respond|reply|replied|called me|texted me|emailed me|invited|interested/.test(text)}
...
export function buildRelationshipInsights(coaches:CoachRelationshipLike[],interactions:InteractionLike[]){return coaches.map((r:any)=>{const relInts=interactions.filter(i=>i&&i.athlete_user_id===r.athlete_user_id&&((r.coach_id&&i.coach_id===r.coach_id)||(!r.coach_id&&r.college_id&&i.college_id===r.college_id)));
 const meaningful=relInts.filter(isMeaningfulContact);const responses=relInts.filter(isCoachResponse);const exactInts=relInts.filter(isExactDatedInteraction).sort((a,b)=>String(exactInteractionDate(b)||'').localeCompare(String(exactInteractionDate(a)||'')));const dates=exactInts.map(exactInteractionDate).filter(Boolean) as string[];const last=dates[0]||r.last_contact_date||null;const age=daysSince(last);
 ...
 const recentPoints=exactInts.filter(i=>daysSince(exactInteractionDate(i))<=60).reduce((n,i)=>n+interactionMeaningScore(i),0);let score=Math.min(70,recentPoints);score+=Math.min(15,responses.length*5);if(age<=7)score+=15;else if(age<=14)score+=10;else if(age<=30)score+=4;else if(age>45&&age!==999)score-=10;score=Math.max(0,Math.min(100,score));
 const recent30=exactInts.filter(i=>daysSince(exactInteractionDate(i))<=30).length;const prior30=exactInts.filter(i=>{const d=daysSince(exactInteractionDate(i));return d>30&&d<=60}).length;let momentum:'Rising'|'Steady'|'Cooling'|'New'='New';if(exactInts.length>=2){if(age>21||recent30<prior30)momentum='Cooling';else if(recent30>prior30||responses.some(i=>{const d=exactInteractionDate(i);return Boolean(d)&&daysSince(d)<=14}))momentum='Rising';else momentum='Steady'}
...
export function athleteEngagementScore(athleteId:string,relationships:RelationshipInsight[],interactions:InteractionLike[],reminders:ReminderLike[]){const rs=relationships.filter(r=>r.athleteUserId===athleteId);const ints=interactions.filter(i=>i&&i.athlete_user_id===athleteId);const recent=ints.filter(i=>{const d=exactInteractionDate(i);return Boolean(d)&&daysSince(d)<=30});const meaningful=recent.filter(isMeaningfulContact);const active=rs.filter(r=>r.daysSinceContact<=14);const responses=recent.filter(isCoachResponse);const open=reminders.filter(r=>r.athlete_user_id===athleteId&&['open','overdue','snoozed'].includes(lower(r.status)));const overdue=open.filter(r=>r.due_date&&r.due_date<new Date().toISOString().slice(0,10));let score=0;score+=Math.min(30,recent.length*4);score+=Math.min(25,meaningful.length*6);score+=Math.min(20,active.length*5);score+=Math.min(15,responses.length*5);score+=rs.length?10:0;score-=Math.min(20,overdue.length*5);return Math.max(0,Math.min(100,score))}

export function cadenceLabel(days:number|null){if(days===null)return 'Not enough exact-date history';if(days<=7)return `About every ${days} day${days===1?'':'s'}`;if(days<=14)return `About every ${days} days`;return `About every ${days} days`}
