...
export function buildRelationshipInsights(coaches:CoachRelationshipLike[],interactions:InteractionLike[]){return coaches.map((r:any)=>{const relInts=interactions.filter(i=>i&&i.athlete_user_id===r.athlete_user_id&&((r.coach_id&&i.coach_id===r.coach_id)||(!r.coach_id&&r.college_id&&i.college_id===r.college_id)));
 ...
 const strength=calculateRelationshipStrength(relInts,r.last_contact_date);const score=Math.max(0,Math.min(100,Math.round((strength.points/30)*100)));
 const momentum:'Rising'|'Steady'|'Cooling'|'New'=strength.momentum==='Growing'?'Rising':strength.momentum==='Steady'?'Steady':strength.momentum==='Cooling'||strength.momentum==='Stalled'?'Cooling':'New';
 const responseSignal=strength.twoWay?'Two-way signals':strength.coachInitiated?'Coach responding':relInts.length?'Outbound only':'No signal yet';
