...
export function contextualNextMove(args:{stage?:string|null;interactions?:any[];events?:any[];lastContact?:string|null}) : ContextualMove {
 const stage=normalizeJourneyStage(args.stage),ints=[...(args.interactions||[])].filter(Boolean).sort((a,b)=>String(b?.date||b?.created_at||'').localeCompare(String(a?.date||a?.created_at||''))),latest=ints[0]||null,text=interactionText(latest),age=daysSince(latest?.date||latest?.created_at||args.lastContact||null),events=[...(args.events||[])].filter(Boolean).sort((a,b)=>String(a?.date||'').localeCompare(String(b?.date||''))),upcoming=events.find(e=>{const n=daysUntil(e?.date);return n>=0&&n<=10}),recent=events.find(e=>{const n=daysUntil(e?.date);return n<0&&n>=-7});
 ...
 if(/watched|came to watch|saw me play|in person/.test(text))return{title:'Thank the coach for watching',reason:'Your latest logged interaction says the coach watched you. A short thank-you with relevant highlights is a strong next move.',starter:'coach_watched',priority:94};
 if(latest&&isCoachResponse(latest))return{title:'Respond to the coach personally',reason:'Your latest interaction shows a coach response. Continue the actual conversation instead of sending a generic update.',starter:'coach_response',priority:92};
 ...
 if(/video|skills|hitting|pitching/.test(text))return{title:'Share a focused video update',reason:'Your recent activity includes a training or video update. Show the staff something specific you are working on.',starter:'new_video',priority:82};
 if(age===999||!ints.length)return{title:stage==='Researching'?'Decide whether to make first contact':'Make the first meaningful contact',reason:stage==='Researching'?'This school is still in Researching. If it becomes a real target, personalize an introduction before reaching out.':'No recruiting contact is logged yet for this relationship.',starter:stage==='Researching'?null:'introduction',priority:75};
 if(age>=28)return{title:'Send a purposeful monthly update',reason:`It has been ${age} days since the last logged contact. Share something meaningful rather than a generic check-in.`,starter:'monthly_update',priority:72};
 ...
 return{title:'Keep the relationship warm with a purposeful touchpoint',reason:'There is recent activity, so avoid emailing just to email. Wait for a useful update, event, video or question.',starter:null,priority:30};
}
