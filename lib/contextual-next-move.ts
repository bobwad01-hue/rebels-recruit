...
export function contextualNextMove(args:{stage?:string|null;interactions?:any[];events?:any[];lastContact?:string|null}) : ContextualMove {
 const stage=normalizeJourneyStage(args.stage),ints=[...(args.interactions||[])].sort((a,b)=>String(b.date||b.created_at||'').localeCompare(String(a.date||a.created_at||''))),latest=ints[0],text=interactionText(latest),age=daysSince(latest?.date||latest?.created_at||args.lastContact||null),events=[...(args.events||[])].filter(Boolean).sort((a,b)=>String(a.date||'').localeCompare(String(b.date||''))),upcoming=events.find(e=>{const n=daysUntil(e.date);return n>=0&&n<=10}),recent=events.find(e=>{const n=daysUntil(e.date);return n<0&&n>=-7});
 ...
 if(isCoachResponse(latest))return{title:'Respond to the coach personally',reason:'Your latest interaction shows a coach response. Continue the actual conversation instead of sending a generic update.',starter:'coach_response',priority:92};
