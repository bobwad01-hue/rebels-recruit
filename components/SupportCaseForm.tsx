...
export default function SupportCaseForm(){
  ...
  useEffect(()=>{(async()=>{
    ...
    const{data,error:e}=await c.from('organization_members').select('organization_id,organizations(name)').eq('user_id',user.id).eq('status','active');
    if(e){setOrgWarning('Organization choices are temporarily unavailable. You can still create a general support case.');return}
    const rows=(data||[]).map((x:any)=>({id:x.organization_id,name:one(x.organizations)?.name||'Organization'}));
    setOrgs(rows);
