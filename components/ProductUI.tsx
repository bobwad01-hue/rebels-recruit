...
export function PageFrame({children,size='7xl',className=''}:{children:React.ReactNode;size?:'5xl'|'6xl'|'7xl';className?:string}){const max=size==='5xl'?'max-w-5xl':size==='6xl'?'max-w-6xl':'max-w-7xl';return <div className={`${max} mx-auto px-4 sm:px-5 md:px-8 py-5 sm:py-7 ${className}`}>{children}</div>}
...
export function LoadingPanel({rows=3,className=''}:{rows?:number;className?:string}){return <div className={`card p-4 sm:p-5 ${className}`} role="status" aria-live="polite"><div className="rr-skeleton rr-skeleton-line w-24"/><div className="rr-skeleton rr-skeleton-title mt-3"/><div className="mt-5 space-y-3">{Array.from({length:rows}).map((_,i)=><div key={i} className="rr-skeleton rr-skeleton-row"/>)}</div><span className="sr-only">Loading</span></div>}

export function StickyMobileAction({href,label,children}:{href?:string;label?:string;children?:React.ReactNode}){return <div className="rr-mobile-sticky-actions">{children||(href&&label?<Link href={href} className="btn btn-red w-full">{label}<ArrowRight size={15}/></Link>:null)}</div>}
...
export function FormSection({title,description,children}:{title:string;description?:string;children:React.ReactNode}){return <section className="card p-4 sm:p-6"><div className="mb-4 sm:mb-5"><h2 className="font-black text-lg">{title}</h2>{description&&<p className="muted text-sm mt-1 leading-relaxed">{description}</p>}</div><div className="space-y-4">{children}</div></section>}
