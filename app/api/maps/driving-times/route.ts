...
export async function POST(req:NextRequest){
 ...
 // Athlete Find Schools uses the signed-in athlete. Parent/guardian Find Schools may request
 // the athlete's origin, but only after we verify active discovery access.
 let profileUserId=user.id;const requestedAthleteId=String(body?.athleteId||'').trim();
 if(requestedAthleteId&&requestedAthleteId!==user.id){
  const {data:access}=await c.from('parent_guardian_access').select('permissions').eq('parent_user_id',user.id).eq('athlete_user_id',requestedAthleteId).eq('status','active').maybeSingle();
  const {data:membership}=await c.from('organization_members').select('role').eq('user_id',user.id).eq('status','active').limit(1).maybeSingle();
  const ownerPreview=membership?.role==='owner'&&body?.previewRole==='parent'&&body?.previewAthlete===requestedAthleteId;
  if(!ownerPreview&&(!access||access.permissions?.view_discovery===false))return NextResponse.json({error:'Find Schools access unavailable.'},{status:403});
  profileUserId=requestedAthleteId;
 }
 const {data:athlete}=await c.from('athlete_profiles').select('home_zip').eq('user_id',profileUserId).maybeSingle();
 const zip=String(athlete?.home_zip||'').trim();const origin=zip?`${zip}, USA`:null;
 if(!origin)return NextResponse.json({configured:true,origin:null,times:{},error:'Add a home ZIP in the athlete profile to see driving times from home.'});
