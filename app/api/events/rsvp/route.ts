...
export async function GET(){
  ...
  const {data,error}=await admin.from('audit_log').select('entity_id,action,metadata,created_at').eq('actor_user_id',user.id).eq('entity_type',ENTITY).order('created_at',{ascending:false});
...
export async function POST(req:NextRequest){
  ...
  if(organizationId){const {data:member}=await admin.from('organization_members').select('user_id').eq('organization_id',organizationId).eq('user_id',user.id).eq('status','active').maybeSingle();if(!member)return NextResponse.json({error:'You do not have access to this organization event.'},{status:403})}
