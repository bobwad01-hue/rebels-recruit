...
export async function POST(request:Request){
  ...
  try{
    ...
    const [me,postsResult,followersResult,followingResult]=await Promise.all([
      xGet(`/users/${account.provider_user_id}?user.fields=profile_image_url`,token),
      xGet(`/users/${account.provider_user_id}/tweets?max_results=20&tweet.fields=created_at,public_metrics`,token),
      xGet(`/users/${account.provider_user_id}/followers?max_results=100&user.fields=profile_image_url`,token),
      xGet(`/users/${account.provider_user_id}/following?max_results=100&user.fields=profile_image_url`,token)
    ]);
    await supabase.from('social_accounts').update({username:me.data?.username||account.username,display_name:me.data?.name||null,profile_image_url:me.data?.profile_image_url||null,status:'connected',last_synced_at:new Date().toISOString()}).eq('id',account.id).eq('athlete_user_id',user.id);
    const posts=postsResult.data||[];const followers=followersResult.data||[];const following=followingResult.data||[];
