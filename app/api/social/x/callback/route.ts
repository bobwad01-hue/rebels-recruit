...
async function xGet(path:string,token:string){
  ...
  if(!res.ok)throw new Error(json?.detail||json?.title||`X API error ${res.status}`);
  return json;
}
...
export async function GET(request:Request){
  ...
  cookieStore.delete('x_oauth_state');cookieStore.delete('x_oauth_verifier');
  ...
  const tokenRes=await fetch('https://api.x.com/2/oauth2/token',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded',Authorization:`Basic ${Buffer.from(`${clientId}:${clientSecret}`).toString('base64')}`},body,cache:'no-store'});
  ...
  try{
    const me=await xGet('/users/me?user.fields=profile_image_url,description,public_metrics',tokenJson.access_token);
    const xUser=me.data;
    ...
    await supabase.from('social_account_secrets').upsert({social_account_id:account.id,athlete_user_id:user.id,access_token:tokenJson.access_token,refresh_token:tokenJson.refresh_token||null,expires_at:tokenJson.expires_in?new Date(Date.now()+tokenJson.expires_in*1000).toISOString():null,updated_at:new Date().toISOString()},{onConflict:'social_account_id'});

    const [postsResult,followersResult,followingResult]=await Promise.allSettled([
      xGet(`/users/${xUser.id}/tweets?max_results=20&tweet.fields=created_at,public_metrics&expansions=attachments.media_keys&media.fields=type,url,preview_image_url`,tokenJson.access_token),
      xGet(`/users/${xUser.id}/followers?max_results=100&user.fields=profile_image_url,public_metrics`,tokenJson.access_token),
      xGet(`/users/${xUser.id}/following?max_results=100&user.fields=profile_image_url,public_metrics`,tokenJson.access_token)
    ]);
    const posts=postsResult.status==='fulfilled'?postsResult.value.data||[]:[];
    const followers=followersResult.status==='fulfilled'?followersResult.value.data||[]:[];
    const following=followingResult.status==='fulfilled'?followingResult.value.data||[]:[];
    await supabase.from('social_posts').delete().eq('social_account_id',account.id);
    if(posts.length)await supabase.from('social_posts').insert(posts.map((p:any)=>({social_account_id:account.id,athlete_user_id:user.id,provider_post_id:p.id,text:p.text||null,posted_at:p.created_at||null,post_url:`https://x.com/${xUser.username}/status/${p.id}`,like_count:p.public_metrics?.like_count||0,reply_count:p.public_metrics?.reply_count||0,repost_count:p.public_metrics?.retweet_count||0,view_count:p.public_metrics?.impression_count||0})));
    const followingIds=new Set(following.map((u:any)=>u.id));
    const followerIds=new Set(followers.map((u:any)=>u.id));
    const merged=new Map<string,any>();
    followers.forEach((u:any)=>merged.set(u.id,{...u,follows_athlete:true,athlete_follows:followingIds.has(u.id)}));
    following.forEach((u:any)=>merged.set(u.id,{...u,follows_athlete:followerIds.has(u.id),athlete_follows:true}));
    await supabase.from('social_connections').delete().eq('social_account_id',account.id);
    const connections=Array.from(merged.values());
    if(connections.length)await supabase.from('social_connections').insert(connections.map((u:any)=>({social_account_id:account.id,athlete_user_id:user.id,provider_user_id:u.id,username:u.username||null,display_name:u.name||null,profile_image_url:u.profile_image_url||null,profile_url:u.username?`https://x.com/${u.username}`:null,follows_athlete:!!u.follows_athlete,athlete_follows:!!u.athlete_follows,last_seen_at:new Date().toISOString()})));
    return NextResponse.redirect(new URL('/social?connected=x',request.url));
