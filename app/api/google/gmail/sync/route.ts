...
export async function POST(req:NextRequest){
 const c=await createClient(); const {data:{user}}=await c.auth.getUser();const pushUser=req.headers.get('x-gmail-push-user'),pushSecret=req.headers.get('x-gmail-push-secret');const trustedPush=!!pushUser&&!!process.env.GMAIL_PUSH_SECRET&&pushSecret===process.env.GMAIL_PUSH_SECRET;const userId=user?.id||(trustedPush?pushUser:null);if(!userId)return NextResponse.json({error:'Please sign in again.'},{status:401});
 try{
  const {accessToken,grantedScopes}=await getGoogleAccessToken(userId,'gmail');
  ...
  for(const item of listing.messages||[]){
   const existing=await admin.from('gmail_recruiting_messages').select('id').eq('athlete_user_id',userId).eq('gmail_message_id',item.id).maybeSingle();
   ...
   const target=normalizeForMatch(intel.summary); const normalizedSubject=normalizeForMatch(subject); const duplicate=(sameDay.data||[]).find((x:any)=>{const n=normalizeForMatch(x.note||'');const s=normalizeForMatch(x.email_subject||'');return s===normalizedSubject&&n&&target&&(n.includes(target.slice(0,120))||target.includes(n.slice(0,120))) });
   ...
   if(intel.nextAction){
    await admin.from('athlete_coaches').update({next_step:intel.nextAction,last_contact_date:receivedAt.slice(0,10)}).eq('athlete_user_id',userId).eq('coach_id',coach.id);
   }
   logged++;
  }
  await admin.from('google_workspace_connections').update({gmail_last_synced_at:new Date().toISOString()}).eq('user_id',userId);
