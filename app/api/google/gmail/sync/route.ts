...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  for(const item of listing.messages||[]){
   ...
   const interaction=await admin.from('interactions').insert({athlete_user_id:userId,actor_user_id:userId,college_id:coach.college_id,coach_id:coach.id,type:'Email Received',initiated_by:'Coach',date:receivedAt.slice(0,10),note:intel.summary}).select('id').single();
