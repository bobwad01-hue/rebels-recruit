"use client";
import { useEffect, useMemo, useState } from "react";
import { useParams } from "next/navigation";
import Link from "next/link";
import {
  Activity,
  ArrowRight,
  CalendarDays,
  CheckCircle2,
  ChevronDown,
  ChevronUp,
  School,
  Target,
} from "lucide-react";
import AppShell from "@/components/AppShell";
import PlayerPhotoUpload from "@/components/PlayerPhotoUpload";
import PageHeader from "@/components/PageHeader";
import SmartNextMoves, {
  buildSmartNextMoves,
} from "@/components/SmartNextMoves";
import { createClient } from "@/lib/supabase-browser";
import { buildRecruitingIntelligence } from "@/lib/recruiting-intelligence";
import { addPreviewToHref, type OwnerPreview } from "@/lib/owner-preview";
const one = (v: any) => (Array.isArray(v) ? v[0] : v);
const fmt = (d: any) =>
  d
    ? new Date(`${String(d).slice(0, 10)}T12:00:00`).toLocaleDateString(
        "en-US",
        { month: "short", day: "numeric", year: "numeric" },
      )
    : "";
export default function Player360() {
  const { id } = useParams<{ id: string }>();
  const c = createClient();
  const [loading, setLoading] = useState(true),
    [error, setError] = useState(""),
    [role, setRole] = useState(""),
    [player, setPlayer] = useState<any>(null),
    [colleges, setColleges] = useState<any[]>([]),
    [coaches, setCoaches] = useState<any[]>([]),
    [interactions, setInteractions] = useState<any[]>([]),
    [reminders, setReminders] = useState<any[]>([]),
    [tasks, setTasks] = useState<any[]>([]),
    [events, setEvents] = useState<any[]>([]),
    [permissions, setPermissions] = useState<any>(null),
    [signatureProfile, setSignatureProfile] = useState<any>({}),
    [teamName, setTeamName] = useState(""),
    [preview, setPreview] = useState<OwnerPreview>({
      active: false,
      role: null,
      athleteId: null,
    }),
    [showConnections, setShowConnections] = useState(false);
  useEffect(() => {
    (async () => {
      if (!id) return;
      const {
        data: { user },
      } = await c.auth.getUser();
      if (!user) {
        setError("Sign in required.");
        setLoading(false);
        return;
      }
      const { data: viewer } = await c
        .from("profiles")
        .select("app_role")
        .eq("id", user.id)
        .single();
      let r = viewer?.app_role || "athlete";
      const sp =
          typeof window !== "undefined"
            ? new URLSearchParams(window.location.search)
            : new URLSearchParams(),
        pr = sp.get("previewRole"),
        pa = sp.get("previewAthlete");
      if (
        r === "owner" &&
        ["athlete", "parent", "advisor"].includes(String(pr)) &&
        pa === id
      ) {
        const { data: memberships } = await c
          .from("organization_members")
          .select("organization_id,role")
          .eq("user_id", user.id)
          .eq("status", "active")
          .limit(10);
        const m = (memberships || []).find((x: any) => x.role === "owner");
        if (m?.role === "owner") {
          const { data: t } = await c
            .from("organization_members")
            .select("user_id")
            .eq("organization_id", m.organization_id)
            .eq("user_id", id)
            .eq("role", "athlete")
            .eq("status", "active")
            .maybeSingle();
          if (t) {
            r = String(pr);
            setPreview({ active: true, role: pr as any, athleteId: id });
            if (r === "parent")
              setPermissions({
                view_discovery: true,
                view_connections: true,
                view_activity: true,
                view_events: true,
                view_fit: true,
                view_progress: true,
                view_videos: true,
                view_game_plan: true,
              });
          }
        }
      }
      setRole(r);
      if (r === "athlete" && user.id !== id && viewer?.app_role !== "owner") {
        setError("You do not have access to this player.");
        setLoading(false);
        return;
      }
      if (r === "parent" && viewer?.app_role !== "owner") {
        const { data: a } = await c
          .from("parent_guardian_access")
          .select("status,permissions")
          .eq("parent_user_id", user.id)
          .eq("athlete_user_id", id)
          .eq("status", "active")
          .maybeSingle();
        if (!a) {
          setError(
            "This athlete has not granted active Parent / Guardian access.",
          );
          setLoading(false);
          return;
        }
        setPermissions(a.permissions || {});
      }
      const [p, ap, cs, ch, int, re, ta, ae, sig, fit] = await Promise.all([
        c
          .from("profiles")
          .select("id,full_name,email,avatar_url,profile_completed_at")
          .eq("id", id)
          .maybeSingle(),
        c.from("athlete_profiles").select("*").eq("user_id", id).maybeSingle(),
        c
          .from("athlete_colleges")
          .select(
            "id,college_id,status,archived_at,fit_rating,colleges(id,name,state,city,division)",
          )
          .eq("athlete_user_id", id),
        c
          .from("athlete_coaches")
          .select(
            "id,college_id,last_contact_date,next_step,relationship_rating,colleges(id,name),college_coaches(id,first_name,last_name,title,email)",
          )
          .eq("athlete_user_id", id),
        c
          .from("interactions")
          .select(
            "id,college_id,coach_id,type,note,date,date_precision,created_at,initiated_by",
          )
          .eq("athlete_user_id", id)
          .order("date", { ascending: false })
          .limit(100),
        c
          .from("reminders")
          .select(
            "id,title,due_date,status,colleges(id,name),college_coaches(id,first_name,last_name)",
          )
          .eq("athlete_user_id", id)
          .limit(50),
        c
          .from("advisor_tasks")
          .select("id,title,description,due_date,status")
          .eq("athlete_user_id", id)
          .limit(50),
        c
          .from("athlete_events")
          .select(
            "id,status,events(id,name,date,type,location,colleges(id,name))",
          )
          .eq("athlete_user_id", id),
        fetch(`/api/profile/email-signature?athlete=${id}`,{cache:"no-store"}).then(async(r)=>({ok:r.ok,data:r.ok?await r.json():null})).catch(()=>({ok:false,data:null})),
        c.from("college_fit_profiles").select("completed_at").eq("user_id", id).maybeSingle(),
      ]);
      const failures = [
        p.error && "player profile",
        ap.error && "athlete details",
        cs.error && "school relationships",
        ch.error && "coach relationships",
        int.error && "recruiting activity",
        re.error && "reminders",
        ta.error && "Next Steps",
        ae.error && "events",
      ].filter(Boolean);
      if (failures.length) {
        console.error("Player 360 load failed", {
          failures,
          errors: [p, ap, cs, ch, int, re, ta, ae]
            .map((result) => result.error?.message)
            .filter(Boolean),
        });
        setError(
          "Player 360 could not load " +
            failures.join(", ") +
            ". Refresh the page and try again.",
        );
        setLoading(false);
        return;
      }
      if (!p.data) {
        setError("Player access is not available.");
        setLoading(false);
        return;
      }
      setPlayer({ ...p.data, ...(ap.data || {}), fit_completed_at: fit.data?.completed_at || null });
      setSignatureProfile(sig?.data?.signature || {});
      if (ap.data?.primary_team_id) {
        const { data: teamRow } = await c.from("teams").select("name").eq("id", ap.data.primary_team_id).maybeSingle();
        setTeamName(teamRow?.name || sig?.data?.signature?.travelTeamName || "");
      } else setTeamName(sig?.data?.signature?.travelTeamName || "");
      setColleges(cs.data || []);
      setCoaches(ch.data || []);
      const collegeMap = new Map(
        (cs.data || []).map((row: any) => [row.college_id, one(row.colleges)]),
      );
      const coachMap = new Map(
        (ch.data || []).map((row: any) => [
          one(row.college_coaches)?.id,
          one(row.college_coaches),
        ]),
      );
      setInteractions(
        (int.data || []).map((row: any) => ({
          ...row,
          colleges: collegeMap.get(row.college_id) || null,
          college_coaches: coachMap.get(row.coach_id) || null,
        })),
      );
      setReminders(re.data || []);
      setTasks(ta.data || []);
      setEvents(
        (ae.data || [])
          .map((x: any) => ({
            ...one(x.events),
            athlete_event_id: x.id,
            status: x.status,
          }))
          .filter((x: any) => x.id),
      );
      setLoading(false);
    })();
  }, [id]);
  const intelligence = useMemo(
    () =>
      buildRecruitingIntelligence({
        colleges,
        coaches,
        interactions,
        reminders,
        tasks,
        events,
        healthProfile:{classYear:player?.class_year||null,profileComplete:!!player?.profile_completed_at,fitComplete:!!player?.fit_completed_at},
      }),
    [colleges, coaches, interactions, reminders, tasks, events, player?.class_year, player?.profile_completed_at, player?.fit_completed_at],
  );
  const moves = useMemo(
    () => buildSmartNextMoves({ tasks, coaches, colleges, events, interactions }),
    [tasks, coaches, colleges, events, interactions],
  );
  const parent = role === "parent",
    athleteView = role === "athlete",
    advisor = role === "advisor" || role === "owner" || role === "admin",
    can = (key: string) => !parent || permissions?.[key] !== false;
  const ph = (href: string) => addPreviewToHref(href, preview),
    athleteQuery = athleteView ? "" : `?athlete=${id}`;
  const parentHref = (href?: string) => {
    let h = href || "/parent/goals";
    if (parent) {
      if (h.startsWith("/game-plan")) h = "/parent/goals";
      else if (
        h === "/events" ||
        h.startsWith("/events?") ||
        h.startsWith("/events#") ||
        h.startsWith("/events/")
      )
        h = "/parent/events";
      else if (
        h === "/connections" ||
        h.startsWith("/connections?") ||
        h.startsWith("/connections#")
      )
        h = "/parent/connections";
      else if (
        h === "/discover" ||
        h.startsWith("/discover?") ||
        h.startsWith("/discover#")
      )
        h = `/parent/discover?athlete=${id}`;
      else if (h.startsWith("/coaches/") || h.startsWith("/colleges/")) {
        const u = new URL(h, "https://preview.local");
        u.searchParams.set("athlete", id);
        h = `${u.pathname}${u.search}${u.hash}`;
      }
    }
    return ph(h);
  };
  const staffHref = (href: string) => {
    let h = href;
    if (h.startsWith("/game-plan")) h = `/messages?tab=tasks&player=${id}`;
    else if (h === "/connections" || h.startsWith("/connections?"))
      h = `/advisors/connections?player=${id}`;
    else if (h === "/discover" || h.startsWith("/discover?"))
      h = `/advisors/connections?player=${id}`;
    else if (h.startsWith("/events/")) h = "/events";
    else if (h.startsWith("/coaches/") || h.startsWith("/colleges/")) {
      const u = new URL(h, "https://preview.local");
      u.searchParams.set("athlete", id);
      h = `${u.pathname}${u.search}${u.hash}`;
    }
    return ph(h);
  };
  const contextHref = (href: string) =>
    parent ? parentHref(href) : advisor ? staffHref(href) : ph(href);
  const metricHref = (label: string) =>
    label === "Recruiting Health"
      ? athleteView
        ? ph("/health")
        : "#recruiting-intelligence"
      : label === "Open Next Steps"
        ? parent
          ? parentHref("/parent/goals")
          : advisor
            ? `#next-moves`
            : ph("/game-plan#next-moves")
        : label === "Upcoming Events"
          ? parent
            ? parentHref("/parent/events")
            : advisor
              ? "/events"
              : ph("/events")
          : parent
            ? parentHref("/parent/connections")
            : advisor
              ? `/advisors/connections?player=${id}`
              : ph("/connections");
  const currentColleges = colleges.filter((x) => !x.archived_at),
    visibleConnections = showConnections
      ? currentColleges
      : currentColleges.slice(0, 5);
  if (loading)
    return (
      <AppShell>
        <div className="max-w-7xl mx-auto px-4 sm:px-5 md:px-8 py-6 sm:py-8">
          <div className="card p-10 text-center muted">
            Building Player 360°...
          </div>
        </div>
      </AppShell>
    );
  if (error)
    return (
      <AppShell>
        <div className="max-w-3xl mx-auto px-5 py-8">
          <div className="card p-5 sm:p-8">
            <h1 className="text-2xl font-black">Access unavailable</h1>
            <p className="muted mt-2">{error}</p>
          </div>
        </div>
      </AppShell>
    );
  return (
    <AppShell>
      <div className="max-w-7xl mx-auto px-4 sm:px-5 md:px-8 py-6">
        <div className="flex items-center gap-4 sm:gap-5 mb-5">
          {(athleteView||advisor)&&!preview.active&&<PlayerPhotoUpload photoOnly athleteId={id} name={player?.full_name||"Player"} initialUrl={player?.avatar_url} onSaved={(avatar_url)=>setPlayer((p:any)=>({...p,avatar_url}))}/>}
          <div className="min-w-0 flex-1"><PageHeader
            title={player?.full_name || "Player"}
            eyebrow="PLAYER 360° SNAPSHOT"
            subtitle={`${player?.class_year || "Class year not set"}${player?.positions?.length ? " · " + player.positions.join(" / ") : ""}`}
          /></div>
        </div>
        <div className="grid grid-cols-2 xl:grid-cols-6 gap-3">
          {[
            ["Recruiting Health", `${intelligence.score}`, intelligence.health],
            ["Active Schools", intelligence.activeSchools, ""],
            ["Target Schools", intelligence.targetSchools, ""],
            ["Coach Relationships", intelligence.coachRelationships, ""],
            ["Open Next Steps", moves.length, ""],
            ["Upcoming Events", intelligence.upcomingEvents, ""],
          ].map(([label, value, sub]: any) => (
            <Metric
              key={label}
              label={label}
              value={value}
              sub={sub}
              href={metricHref(label)}
            />
          ))}
        </div>
        <div className="grid xl:grid-cols-3 gap-6 mt-6">
          <section
            id="recruiting-intelligence"
            className="card p-5 lg:col-span-2 scroll-mt-24"
          >
            <div className="flex items-center justify-between">
              <div>
                <div className="rr-eyebrow">RECRUITING INTELLIGENCE</div>
                <h2 className="font-black text-xl mt-1">
                  What matters right now
                </h2>
                <p className="muted text-sm mt-1">
                  Signals surface what deserves attention and why, based on relationships, recruiting activity, open work and upcoming events.
                </p>
              </div>
              <Target size={22} />
            </div>
            {intelligence.topPriority ? (
              <div className="mt-4 rr-priority-card p-4">
                <div className="font-black">
                  {parent ? "What to support next" : advisor ? "Priority signal" : "Do this next"}:{" "}
                  {intelligence.topPriority.title}
                </div>
                <p className="text-sm mt-1">
                  {intelligence.topPriority.detail}
                </p>
                {athleteView &&
                  intelligence.topPriority.href &&
                  !preview.active && (
                    <Link
                      href={intelligence.topPriority.href}
                      className="inline-flex items-center gap-1 text-sm font-black mt-3"
                    >
                      Go to Next Step <ArrowRight size={15} />
                    </Link>
                  )}
                {advisor && !preview.active && (
                  <Link
                    href={`/messages?tab=tasks&player=${id}&title=${encodeURIComponent(intelligence.topPriority.title)}&description=${encodeURIComponent(intelligence.topPriority.detail)}`}
                    className="inline-flex items-center gap-1 text-sm font-black mt-3"
                  >
                    Assign Next Step <ArrowRight size={15} />
                  </Link>
                )}
                {parent && (
                  <Link
                    href={parentHref(intelligence.topPriority.href)}
                    className="inline-flex items-center gap-1 text-sm font-black mt-3"
                  >
                    See Details <ArrowRight size={15} />
                  </Link>
                )}
              </div>
            ) : (
              <div className="rr-empty-state mt-4">
                <div className="font-black">
                  No urgent recruiting signal right now.
                </div>
                <p className="muted text-sm mt-1">
                  Keep working the current plan and upcoming opportunities.
                </p>
              </div>
            )}
            <div className="mt-4 divide-y">
              {intelligence.signals.slice(1, 5).map((s) =>
                s.href ? (
                  <Link
                    key={s.id}
                    href={contextHref(s.href)}
                    className="block py-3 hover:bg-slate-50 -mx-2 px-2 rounded-lg"
                  >
                    <div className="font-bold text-sm">{s.title}</div>
                    <div className="muted text-xs mt-1">{s.detail}</div>
                    <div className="text-xs font-black mt-2">
                      Review Details →
                    </div>
                  </Link>
                ) : (
                  <div key={s.id} className="py-3">
                    <div className="font-bold text-sm">{s.title}</div>
                    <div className="muted text-xs mt-1">{s.detail}</div>
                  </div>
                ),
              )}
            </div>
            {athleteView && (
              <Link href={ph("/health")} className="btn mt-4">
                See how Recruiting Health works <ArrowRight size={14} />
              </Link>
            )}
          </section>
          <section className="card p-5">
            <div className="rr-eyebrow">PLAYER PROFILE</div>
            <h2 className="font-black text-lg">Recruiting essentials</h2>
            <div className="mt-4 space-y-3 text-sm">
              <Row k="Class" v={player?.class_year || "Not set"} />
              <Row k="Position" v={player?.positions?.join(" / ") || "Not set"} />
              <Row k="Jersey #" v={player?.jersey_number || "Not set"} />
              <Row k="GPA" v={player?.gpa || "Not set"} />
              <Row k="Home" v={[player?.home_city||signatureProfile?.highSchoolCity,player?.primary_state||signatureProfile?.highSchoolState].filter(Boolean).join(", ") || player?.home_zip || "Not set"} />
              <Row k="Interested majors" v={player?.interested_majors?.join(", ") || "Not set"} />
              <div className="pt-3 mt-3 border-t"><div className="text-[10px] font-black uppercase tracking-[.14em] text-slate-500">High School</div></div>
              <Row k="School" v={player?.school_name || "Not set"} />
              <Row k="Coach" v={signatureProfile?.highSchoolCoachName || "Not set"} />
              <Row k="Coach phone" v={signatureProfile?.highSchoolCoachPhone || "Not set"} />
              <Row k="Coach email" v={signatureProfile?.highSchoolCoachEmail || "Not set"} />
              <div className="pt-3 mt-3 border-t"><div className="text-[10px] font-black uppercase tracking-[.14em] text-slate-500">Travel Team</div></div>
              <Row k="Team" v={teamName || signatureProfile?.travelTeamName || "Not set"} />
              <Row k="Coach" v={signatureProfile?.travelTeamCoachName || "Not set"} />
              <Row k="Coach phone" v={signatureProfile?.travelTeamCoachPhone || "Not set"} />
              <Row k="Coach email" v={signatureProfile?.travelTeamCoachEmail || "Not set"} />
            </div>
            {athleteView && !preview.active && (
              <Link href="/profile" className="btn w-full mt-5">
                Update Profile
              </Link>
            )}
          </section>
        </div>
        {can("view_game_plan") && (
          <section id="next-moves" className="card p-5 mt-6 scroll-mt-24">
            <div className="rr-eyebrow">
              {parent ? "READ-ONLY WORK VIEW" : "NEXT STEPS"}
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle2 size={19} />
              <h2 className="font-black text-lg">
                {parent ? "Current Next Steps" : advisor ? `${String(player?.full_name||"Player").split(" ")[0]}'s Next Steps` : "What should happen next"}
              </h2>
            </div>
            <p className="muted text-sm mt-1">
              {parent
                ? "Read-only view of what the athlete is working on. Use this to support, not take over."
                : advisor
                  ? `Work ${String(player?.full_name||"the player").split(" ")[0]} is responsible for completing. Review progress here, and assign a new Next Step when coaching support is needed.`
                  : "Prioritized from your current recruiting state. As relationships, events and Next Steps change, this list changes too."}
            </p>
            <div className="mt-4">
              {parent || advisor ? (
                <div className="space-y-2">
                  {moves.slice(0, 6).map((m: any) => (
                    <Link
                      href={parent ? parentHref(m.href) : staffHref(m.href)}
                      key={m.id || m.title}
                      className="block border rounded-xl p-4 rr-interactive-card"
                    >
                      <div className="font-bold text-sm">{m.title}</div>
                      {m.detail && (
                        <div className="muted text-xs mt-1">{m.detail}</div>
                      )}
                      <div className="text-xs font-black mt-2">
                        Review Details →
                      </div>
                    </Link>
                  ))}
                </div>
              ) : (
                <SmartNextMoves
                  moves={moves.slice(0, 6)}
                  hrefTransform={advisor ? staffHref : ph}
                />
              )}
            </div>
          </section>
        )}
        <div className="grid xl:grid-cols-2 gap-6 mt-6">
          {can("view_connections") && (
            <section className="card p-5">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="rr-eyebrow">CONNECTIONS</div>
                  <div className="flex items-center gap-2">
                    <School size={18} />
                    <h2 className="font-black text-lg">
                      Priority Schools ({currentColleges.length})
                    </h2>
                  </div>
                </div>
                {currentColleges.length > 5 && (
                  <button
                    type="button"
                    onClick={() => setShowConnections((v) => !v)}
                    className="text-sm font-black hover:underline"
                  >
                    {showConnections ? "View Less" : "View All"}
                  </button>
                )}
              </div>
              <div className="mt-3 divide-y">
                {visibleConnections.map((x) => {
                  const s = one(x.colleges);
                  return (
                    <Link
                      key={x.id}
                      href={contextHref(`/colleges/${s?.id}${athleteQuery}`)}
                      className="py-3 flex justify-between gap-3 hover:bg-slate-50 -mx-2 px-2 rounded-lg"
                    >
                      <span className="font-bold text-sm">
                        {s?.name || "School"}
                      </span>
                      <span className="flex items-center gap-2">
                        <span className="muted text-xs">
                          {x.status || "Researching"}
                        </span>
                        <ArrowRight size={13} />
                      </span>
                    </Link>
                  );
                })}
                {!visibleConnections.length && (
                  <div className="rr-empty-state">
                    <div className="font-black">
                      No current school relationships.
                    </div>
                  </div>
                )}
              </div>
              {currentColleges.length > 5 && (
                <button
                  type="button"
                  onClick={() => setShowConnections((v) => !v)}
                  className="btn w-full mt-3"
                >
                  {showConnections ? (
                    <ChevronUp size={16} />
                  ) : (
                    <ChevronDown size={16} />
                  )}{" "}
                  {showConnections
                    ? "View Less"
                    : `View All ${currentColleges.length} Schools`}
                </button>
              )}
              <Link
                href={
                  parent
                    ? parentHref("/parent/connections")
                    : advisor
                      ? `/advisors/connections?player=${id}`
                      : ph("/connections")
                }
                className="btn w-full mt-3"
              >
                Review Connections
              </Link>
            </section>
          )}
          {can("view_activity") && (
            <section className="card p-5">
              <div className="rr-eyebrow">JOURNEY</div>
              <div className="flex items-center gap-2">
                <Activity size={18} />
                <h2 className="font-black text-lg">
                  Recent Recruiting Activity
                </h2>
              </div>
              <div className="mt-3 divide-y">
                {interactions.slice(0, 8).map((x) => {
                  const school = one(x.colleges),
                    coach = one(x.college_coaches);
                  const activityHref = parent
                    ? parentHref("/parent/journey")
                    : advisor
                      ? `/advisors/activity?player=${id}`
                      : ph(`/activity/${x.id}`);
                  return (
                    <div key={x.id} className="py-3">
                      <Link
                        href={activityHref}
                        className="font-bold text-sm hover:text-red-700"
                      >
                        {x.type}
                      </Link>
                      <div className="muted text-xs flex flex-wrap gap-1">
                        {school?.id ? (
                          <Link
                            href={contextHref(
                              `/colleges/${school.id}${athleteQuery}`,
                            )}
                            className="hover:underline font-semibold"
                          >
                            {school.name}
                          </Link>
                        ) : (
                          <span>General recruiting</span>
                        )}
                        {coach?.id && (
                          <>
                            <span>·</span>
                            <Link
                              href={contextHref(
                                `/coaches/${coach.id}${athleteQuery}`,
                              )}
                              className="hover:underline font-semibold"
                            >
                              {[coach.first_name, coach.last_name]
                                .filter(Boolean)
                                .join(" ")}
                            </Link>
                          </>
                        )}
                        <span>
                          ·{" "}
                          {x.date_precision === "exact" && x.date
                            ? fmt(x.date)
                            : x.date_precision === "month"
                              ? "Month known"
                              : x.date_precision === "year"
                                ? "Year known"
                                : "Date unknown"}
                        </span>
                      </div>
                    </div>
                  );
                })}
              </div>
              <Link
                href={
                  parent
                    ? parentHref("/parent/journey")
                    : advisor
                      ? `/advisors/activity?player=${id}`
                      : ph("/journey")
                }
                className="btn w-full mt-3"
              >
                Open Full Journey
              </Link>
            </section>
          )}
        </div>
        {can("view_events") && (
          <section className="card p-5 mt-6">
            <div className="rr-eyebrow">UPCOMING EVENTS</div>
            <div className="flex items-center gap-2">
              <CalendarDays size={18} />
              <h2 className="font-black text-lg">Prepare for what is coming</h2>
            </div>
            <div className="grid md:grid-cols-3 gap-3 mt-4">
              {events
                .filter((e) => e.date >= new Date().toISOString().slice(0, 10))
                .slice(0, 6)
                .map((e) => {
                  const href = parent
                    ? parentHref("/parent/events")
                    : advisor
                      ? "/events"
                      : ph(`/events/${e.id}/prep`);
                  return (
                    <Link
                      href={href}
                      key={e.id}
                      className="block border rounded-xl p-3 rr-interactive-card"
                    >
                      <div className="font-bold text-sm">{e.name}</div>
                      <div className="muted text-xs mt-1">
                        {fmt(e.date)} ·{" "}
                        {e.status === "going"
                          ? "Going"
                          : e.status === "not_going"
                            ? "Not Going"
                            : "Considering"}
                      </div>
                      <div className="text-xs font-black mt-2">
                        {parent
                          ? "Review Event"
                          : advisor
                            ? "Review Attendance"
                            : "Open Prep"}{" "}
                        →
                      </div>
                    </Link>
                  );
                })}
            </div>
            <Link
              href={parent ? parentHref("/parent/events") : "/events"}
              className="btn w-full mt-4"
            >
              Review All Events
            </Link>
          </section>
        )}
      </div>
    </AppShell>
  );
}
function Metric({
  label,
  value,
  sub,
  href,
}: {
  label: string;
  value: any;
  sub?: string;
  href: string;
}) {
  return (
    <Link href={href} className="rr-metric-card p-4 rr-interactive-card">
      <div className="rr-metric-label">{label}</div>
      <div className="rr-metric-value">{value}</div>
      {sub && <div className="text-xs font-bold mt-1">{sub}</div>}
      <div className="text-[11px] font-black mt-2">Review →</div>
    </Link>
  );
}
function Row({ k, v }: { k: string; v: any }) {
  return (
    <div>
      <div className="muted text-xs">{k}</div>
      <div className="font-bold mt-1">{v}</div>
    </div>
  );
}
