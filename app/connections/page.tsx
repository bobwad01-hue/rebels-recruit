...
export default async function Connections({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 try{
  ...
  return <AppShell><div className="max-w-7xl mx-auto px-4 sm:px-5 md:px-8 py-6"><PageHeader eyebrow="YOUR RECRUITING RELATIONSHIPS" title="Connections" subtitle="Keep the schools you are pursuing and the coaches you know in one place. Open a school or coach to see the relationship, recent activity and what to do next."/><div className="mb-5 rounded-xl border bg-slate-50 px-4 py-3 text-sm text-slate-700"><b>Why Connections matter:</b> Recruiting is built on relationships. Keeping each school and coach connected to your conversations helps you remember where things stand and follow up with purpose.</div><div className="[&>div:first-child]:hidden [&>div:nth-child(2)>div:nth-child(3)]:hidden"><CollegesCoachesBoard colleges={colleges.data||[]} coaches={coaches.data||[]} masterColleges={masterColleges} masterCoaches={masterCoaches.data||[]}/></div></div></AppShell>;
 }catch(error){
