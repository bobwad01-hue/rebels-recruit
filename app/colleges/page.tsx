...
export default async function Colleges(){
 ...
 const [colleges,coaches,masterColleges,masterCoaches]=await Promise.all([
  supabase.from('athlete_colleges').select('id,status,fit_rating,created_at,archived_at,archived_reason,colleges(id,name,division,state,city,conference)').eq('athlete_user_id',user?.id).order('created_at',{ascending:false}),
 ...
 return <AppShell><div className="max-w-7xl mx-auto px-5 md:px-8 py-6"><CollegesCoachesBoard colleges={colleges.data||[]} coaches={coaches.data||[]} masterColleges={masterColleges||[]} masterCoaches={masterCoaches.data||[]}/></div></AppShell>
}
