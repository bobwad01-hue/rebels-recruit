...
export default async function Colleges(){
 ...
 const [colleges,coaches,masterColleges,masterCoaches,interactions]=await Promise.all([
  ...
  supabase.from('interactions').select('id,college_id,type,date,created_at,note,initiated_by').eq('athlete_user_id',user?.id).order('date',{ascending:false}).limit(1000)
 ]);
 return <AppShell><div className="max-w-7xl mx-auto px-5 md:px-8 py-6"><CollegesCoachesBoard colleges={colleges.data||[]} coaches={coaches.data||[]} masterColleges={masterColleges||[]} masterCoaches={masterCoaches.data||[]} interactions={interactions.data||[]}/></div></AppShell>
}
