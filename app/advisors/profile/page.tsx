...
export default function AdvisorProfile() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: p, error: profileError } = await c
        .from("profiles")
        .select(
          "full_name,email,phone,advisor_type,timezone,profile_completed_at",
        )
        .eq("id", user.id)
        .single();
      if (profileError) {
      ...
      setPhone(p?.phone || "");
      setType(p?.advisor_type || "Advisor");
  ...
  async function save(e: React.FormEvent) {
    ...
    if (!name.trim() || !cleanEmail || !phone.trim() || !type || !timezone) {
        ...
        <div className="card p-6">
          ...
          {!loading && (
            <form onSubmit={save} className="grid md:grid-cols-2 gap-4">
              ...
              <label className="block">
                <span className="text-sm font-bold">I am a *</span>
                <select
                  className="input mt-1"
                  value={type}
