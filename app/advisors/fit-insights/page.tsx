...
export default function FitInsights(){
 ...
 async function load(){
  ...
  try{
   const {data:{user},error:authError}=await c.auth.getUser();if(authError)throw authError;if(!user){setError('Sign in to view College Fit Insights.');return}
  ...
  }catch(e:any){console.error('College Fit Insights load failed',e);setError('We could not load College Fit Insights. Your recruiting data has not been changed. Check your connection and try again.')}finally{setLoading(false)}
 }
 ...
 if(loading)return <AppShell><PageFrame size="5xl"><StatePanel title="Loading College Fit Insights" description="We’re gathering the players, teams, and Fit Profiles you’re allowed to view."/></PageFrame></AppShell>;
 if(error)return <AppShell><PageFrame size="5xl"><StatePanel tone="error" title="College Fit Insights unavailable" description={error} action={<button className="btn" onClick={load}>Try again</button>}/></PageFrame></AppShell>;
 return <AppShell><PageFrame>
  <PageHeader title="College Fit Insights" subtitle="See what matters to your players at the organization, team, age-group, and individual level."/>
  {!players.length ? (
   <EmptyState title="No players are in this view yet" description="When athletes are assigned to you—or added to the organization if you have organization-wide access—their College Fit preferences will appear here." href="/advisors/access" actionLabel="Review player access"/>
  ) : (
   <>
