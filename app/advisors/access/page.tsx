...
export default function AdvisorAccessPage() {
  ...
  async function load() {
    ...
    const [membersResult, assignmentResult] = await Promise.all([
      ...
      c
        ...
        .select(
          "id,athlete_user_id,advisor_user_id,status,invited_at,responded_at",
        )
        .eq("organization_id", me.organization_id)
    ...
    const as = assignmentResult.data || [];
    const athleteIds = (members || [])
      .filter((m: any) => m.role === "athlete")
      .map((m: any) => String(m.user_id));
    const profileResult = athleteIds.length
      ? await c
          .from("profiles")
    ...
    if (profileResult.error) {
      setLoadError("Player names and email addresses could not be loaded.");
      setLoading(false);
      return;
    }
  ...
  async function requestAccess(athleteId: string) {
    ...
    if (existing) {
      ...
      const r = await c.from("athlete_advisor_assignments").insert({
        athlete_user_id: athleteId,
        advisor_user_id: viewerId,
        organization_id: orgId,
        status: "pending",
        relationship_type: "Recruiting Advisor",
      });
      error = r.error;
    }
    if (error) {
      ...
      setMessage(
        "Access request sent. The player must approve it before you can view and support their recruiting work.",
      );
      await load();
    }
    setBusy("");
  }
  async function cancelRequest(athleteId: string) {
    const existing = byAthlete.get(athleteId);
    if (!existing) return;
    setBusy(athleteId);
    setMessage("");
  ...
  return (
    <AppShell>
      <div className="max-w-6xl mx-auto px-4 sm:px-5 md:px-8 py-5 sm:py-6">
        <Link
          href="/advisors"
          className="text-sm font-bold inline-flex items-center gap-2 mb-4"
        >
          <ArrowLeft size={15} /> Back to Advisor Home
        </Link>
        <PageHeader
          eyebrow="WHO YOU CAN SUPPORT"
          title="Player Access"
          subtitle="Request access to players you advise. The player must approve the connection before you can view their recruiting work."
        />
        <div className="mb-5 rounded-xl border bg-slate-50 px-4 py-3 text-sm text-slate-700">
          <b>Why approval matters:</b> Recruiting belongs to the player. Access
          lets you help with their schools, coach relationships, activity and
          Next Steps without taking control away from them.
        </div>
        {loadError ? (
          <div role="alert" className="card border-red-200 bg-red-50 p-5">
            <div className="font-black">Player access could not be loaded</div>
          ...
          <>
            ...
            <div className="card p-4 sm:p-5">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                  ...
                  <p className="muted text-sm mt-1">
                    ...
                    player’s recruiting picture and where your help may be
                    useful.
                  </p>
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-3 mt-5">
                {filtered.map((p) => {
                  const a = byAthlete.get(String(p.id));
                  ...
                  return (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="muted text-sm truncate">{p.email}</div>
                        <div className="mt-2">
  ...
  if (status === "active")
    return (
      <span className="status-pill bg-green-50 text-green-700">
        <ShieldCheck size={13} /> Ready to support
      </span>
    );
  if (status === "pending")
    return (
      <span className="status-pill bg-amber-50 text-amber-800">
        <Clock3 size={13} /> Waiting for player approval
      </span>
    );
  if (status === "declined")
    return (
