...
export default function SocialPage(){
  ...
  useEffect(()=>{load();const params=new URLSearchParams(window.location.search);const message=connectionMessage(params.get('error'));if(message)setError(message);if(params.get('connected')==='x')setError('X connected successfully. Your latest X data is now available.');},[]);
  const connect=()=>{setConnecting(true);window.location.assign('/api/social/x/connect')};
  ...
  const followers=connections.filter(c=>c.follows_athlete);const following=connections.filter(c=>c.athlete_follows);const notFollowingBack=following.filter(c=>!c.follows_athlete);const lastPost=posts[0];const engagement=posts.reduce((sum,p)=>sum+p.like_count+p.reply_count+p.repost_count,0);

  return <AppShell><div className="max-w-7xl mx-auto px-5 md:px-8 py-6">
    <PageHeader title="Socials" subtitle="Keep your recruiting presence and coach connections in one place." action={<button onClick={refresh} className="btn btn-outline" disabled={refreshing}><RefreshCw size={17} className={refreshing?'animate-spin':''}/> Refresh</button>}/>
    {error&&<div className={`mb-5 rounded-xl px-4 py-3 text-sm ${error.includes('successfully')?'border border-green-200 bg-green-50 text-green-700':'border border-red-200 bg-red-50 text-red-700'}`}>{error}</div>}
    <section className="card p-5 mb-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        {account?<div className="flex flex-wrap items-center gap-3"><span className={`inline-flex items-center gap-1.5 text-sm font-bold ${account.status==='needs_reauth'?'text-red-600':''}`}>{account.status==='connected'?<CheckCircle2 size={17}/>:<RefreshCw size={16}/>} {account.status==='connected'?'Connected':'Needs reauthorization'} {account.username&&`@${account.username}`}</span><Link href={account.profile_url||'#'} target="_blank" className="text-sm font-bold inline-flex items-center gap-1">View profile <ExternalLink size={14}/></Link></div>:<button onClick={connect} disabled={connecting} className="btn btn-red"><X size={17}/>{connecting?'Connecting…':'Connect X'}</button>}
      </div>
      {!account&&<div className="mt-4 rounded-xl bg-slate-50 p-4 text-sm muted">Connect your X account to bring in your recent posts, followers, following, and recruiting-related activity.</div>}
      {account&&<div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-5"><div className="rounded-xl bg-slate-50 p-4"><div className="muted text-xs">Followers</div><div className="font-black text-2xl mt-1">{followers.length}</div></div><div className="rounded-xl bg-slate-50 p-4"><div className="muted text-xs">Following</div><div className="font-black text-2xl mt-1">{following.length}</div></div><div className="rounded-xl bg-slate-50 p-4"><div className="muted text-xs">Don’t follow back</div><div className="font-black text-2xl mt-1">{notFollowingBack.length}</div></div><div className="rounded-xl bg-slate-50 p-4"><div className="muted text-xs">Post engagement</div><div className="font-black text-2xl mt-1">{engagement}</div></div></div>}
    ...
    <section className="card p-5 mt-6"><div className="flex items-center gap-3"><div className="h-10 w-10 rounded-xl bg-slate-100 flex items-center justify-center"><Instagram size={19}/></div><div><h2 className="font-black">Instagram & TikTok</h2><p className="muted text-sm">Coming next in the same Socials system.</p></div></div><div className="grid md:grid-cols-2 gap-4 mt-4"><div className="rounded-xl border p-4"><div className="font-bold">Instagram</div><p className="muted text-sm mt-1">Posts, followers, following and recruiting engagement.</p></div><div className="rounded-xl border p-4"><div className="font-bold">TikTok</div><p className="muted text-sm mt-1">Posting history, audience growth and recruiting activity.</p></div></div></section>
  </div></AppShell>
}
