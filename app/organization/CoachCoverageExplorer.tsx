...
export default function CoachCoverageExplorer({rows,activity,pm,csv}:Props){
 ...
 return <section className="card p-4 sm:p-5"><div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3"><div><div className="rr-eyebrow">COACH RELATIONSHIPS</div><h2 className="rr-section-title">Coach Coverage</h2><p className="rr-section-subtitle">See which coaches are connected to your players, which schools they represent, and where staff may need more relationship coverage.</p></div><button className="btn self-start sm:self-auto" onClick={()=>csv('organization-coaches.csv',filtered.map(g=>({coach:g.name,schools:[...g.schools.values()].join('; '),players:g.players.size,recorded_activity:g.interactions,last_recorded_contact:g.recent||''})))}><Download size={16}/>Export CSV</button></div>
 <div className="rounded-xl border bg-slate-50 px-4 py-3 text-sm text-slate-700 mt-4"><b>Use coverage to find gaps.</b> A player can have a school in Connections without knowing the right coach yet. Filter by school or player to see where an introduction, contact update or relationship check may be needed.</div>
 <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-2 mt-4">
  ...
  <select aria-label="Sort coach coverage" className="input" value={sort} onChange={e=>setSort(e.target.value as any)}><option value="az">A–Z</option><option value="players">Most Players</option><option value="interactions">Most Recorded Activity</option><option value="recent">Most Recent Contact</option></select>
 </div>
