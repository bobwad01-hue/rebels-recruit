...
export default function CoachCoverageExplorer({rows,activity,pm,csv}:Props){
 ...
 const groups=useMemo(()=>{
  ...
  for(const r of rows){
   ...
   g.rows.push(r);g.players.add(r.athlete_user_id);if(r.college_id&&college?.name)g.schools.set(r.college_id,college.name);if(r.last_contact_date&&r.last_contact_date>g.recent)g.recent=r.last_contact_date;by.set(r.coach_id,g);
  }
 ...
 const schools=useMemo(()=>[...new Map(rows.map(r=>[r.college_id,one(r.colleges)?.name]).filter(([id,name])=>id&&name)).entries()].sort((a:any,b:any)=>String(a[1]).localeCompare(String(b[1]))),[rows]);
 const players=useMemo(()=>[...new Set(rows.map(r=>r.athlete_user_id))].map(id=>[id,pm.get(id)?.full_name||pm.get(id)?.email||'Player']).sort((a:any,b:any)=>String(a[1]).localeCompare(String(b[1]))),[rows,pm]);
 ...
 <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-2 mt-4">
  ...
  <select aria-label="Filter coach coverage by school" className="input" value={school} onChange={e=>setSchool(e.target.value)}><option value="">All Schools</option>{schools.map(([id,name]:any)=><option key={id} value={id}>{name}</option>)}</select>
  <select aria-label="Filter coach coverage by player" className="input" value={player} onChange={e=>setPlayer(e.target.value)}><option value="">All Players</option>{players.map(([id,name]:any)=><option key={id} value={id}>{name}</option>)}</select>
