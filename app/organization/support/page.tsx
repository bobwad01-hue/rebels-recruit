...
const orgLabel=(v:any)=>{const row=Array.isArray(v)?v[0]:v;return row?.name||'Organization'};
...
export default async function SupportPage({searchParams}:{searchParams:Promise<{organization?:string}>}){
  ...
  const me:any=staffMemberships.find((m:any)=>m.organization_id===requested)||staffMemberships[0];
  const orgName=orgLabel(me.organizations);
